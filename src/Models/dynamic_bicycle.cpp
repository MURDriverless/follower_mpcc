...
State DynamicBicycleModel::predictContinuous(const State &xk, const Input &uk) const {
    ...
    const double virtual_input = xk(IndexMap.virtual_input);
    // Destructure input
    const double d_accel_D = uk(IndexMap.d_accel_D);
    const double d_steering_angle = uk(IndexMap.d_steering_angle);
    const double d_virtual_input = uk(IndexMap.d_virtual_input);
...
//    const double tau_TV = (wz_target - wz) * P_TV;
    ...
    xdot(IndexMap.virtual_state) = virtual_input;
    xdot(IndexMap.accel_D) = d_accel_D;
    xdot(IndexMap.steering_angle) = d_steering_angle;
    xdot(IndexMap.virtual_input) = d_virtual_input;
    return xdot;
}
...
LinModelMatrix DynamicBicycleModel::calcContinuousJacobian(const State &xk, const Input &uk) const {
    ...
    // Column 7 (virtual state): all zero
    // Column 8 (accel_D):
    ...
    // Column 10 (virtual input):
    Ac(IndexMap.virtual_state, IndexMap.virtual_input) = 1.0;
    ...
    // Column 3 (d_virtual_input):
    Bc(IndexMap.virtual_input, IndexMap.d_virtual_input) = 1.0;
